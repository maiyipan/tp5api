...
class Openapi{
    ...
    public function __construct($config){
        isset($config['source']) ? $this->_source = $config['source'] : trigger_error('source name required', E_USER_ERROR);
        isset($config['secret']) ? $this->_secret = $config['secret'] : trigger_error('secret required', E_USER_ERROR);
        isset($config['url']) ? $this->_url = $config['url'] : trigger_error('url equired', E_USER_ERROR);
        isset($config['version']) && $this->_version = $config['version'];
        isset($config['encrypt']) && $this->_encrypt = $config['encrypt'];
        isset($config['expire']) && $this->_expireTime = intval($config['expire']);
        isset($config['timeout']) && $this->_httpTimeout = intval($config['timeout']);
        isset($config['retry']) && $this->_httpRetry = intval($config['retry']);
    }
     ...
     * 发送命令
     */
    public function send($cmd, $body ,&$reData){
        $this->_reset();
//         dump($cmd);
//         dump($body);
        $req = $this->_buildCmd($cmd, $body);
//         dump($req);
//         $res = $this->_sendReq($req);
        $res = $this->httpPostGetRequest($this->_url,$req,'POST','original');
...
//             curl_setopt($curl, CURLOPT_HTTPHEADER, $headers);
     ...
     */
    function httpPostGetRequest($url,$data=array(),$method = "GET",$dataType='arr'){
    	$ch = curl_init();
